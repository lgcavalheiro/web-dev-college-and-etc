...
document.body.onloadstart = coreUtils.testNavigation(
  "professor",
  loadAllGrades
);
...
document.getElementById(
  "user-info"
).innerHTML = `Professor ${localStorage.getItem(
  "name"
...
function injectStudentForm(id) {
  ...
  ).innerHTML += `${SF.studentForm(data, "/grades", "PUT", "studentForm")}`;
...
function injectGradeForm() {
  ...
  document.querySelector(
    "div[id=modal-content] div"
  ).innerHTML += `${SF.studentForm({}, "/grades", "POST", "gradeForm")}`;
...
function injectButtons(id, name) {
  return `
    <div class="card-aluno-header">
        <h3>${name} - ${id}</h3>
        <button id="modal-btn" onclick="showModal(${id}, 'update')">Editar notas</button>
        <button id="modal-btn" onclick="showModal(${id}, 'deleteGrade')">Deletar aluno</button>
    </div>
    `;
}
...
window.switchTab = function (tab) {
  let hide = tab == "grades-wrapper" ? "dashboard" : "grades-wrapper";
